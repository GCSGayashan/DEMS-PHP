...
final class ArpaWorkflowQueueTest
{
    ...
    private function workflowTest():void
    {
        ...
        try{
            ...
            $historicalReviewId=$this->uuid();$this->pdo->prepare("INSERT INTO arpa_appointment_stage_review(id,entity_type,request_id,review_stage,review_information,remarks,updated_by) VALUES(?,'DIVISION',?,'DISTRICT','Historical District review information','Retained read-only review evidence',?)")->execute([$historicalReviewId,$request,$districtSubject]);
            $historicalReview=$this->row("SELECT sr.*,u.username updated_by_name FROM arpa_appointment_stage_review sr JOIN system_user u ON u.id=sr.updated_by WHERE sr.id=?",[$historicalReviewId]);
            $this->useContext($districtAdmin,'DISTRICT_ADMIN');$approvalRequest=$read->workflowRequestDetail('division',$request);ob_start();(static function(array $request,array $stageReviews):void{$entity='division';$workflowHistory=[];$impact=[];require BASE_PATH.'/app/Views/arpa_appointments/request_detail.php';})($approvalRequest??[],[$historicalReview]);$historicalReviewHtml=(string)ob_get_clean();
            $this->same(true,str_contains($historicalReviewHtml,'Historical District review information')&&str_contains($historicalReviewHtml,'Retained read-only review evidence'),'historical Stage Review information remains safely visible as read-only evidence');
            $this->same(false,str_contains($historicalReviewHtml,'Enter District Review'),'historical Stage Review evidence does not restore the obsolete edit action');$this->useContext($this->asctest,'ASC_SUBJECT_OFFICER');
            ...
            $this->useContext($this->asctest,'ASC_SUBJECT_OFFICER');
            $service->workflow('division',$request,'VERIFY','NATIONAL','Direct National verification remarks',$nationalSubject);
            $this->same('Direct National verification remarks',(string)$this->scalar("SELECT comments FROM arpa_appointment_workflow_action WHERE request_id=? AND action='VERIFY' AND stage='NATIONAL' ORDER BY id DESC LIMIT 1",[$request]),'optional National verification remarks are retained in workflow history');
