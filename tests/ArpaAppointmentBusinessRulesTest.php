...
final class ArpaAppointmentBusinessRulesTest
{
    ...
    private function testRules():void
    {
        ...
        $editedPermanent=$this->request($permanentOfficer,'PERMANENT',$asc,$divisions[9],$today)+['remarks'=>'Submitted maker correction'];
        $service->updateAndResubmitRequest('division',$permanentRequest,$editedPermanent,$actor);
        $this->same('SUBMITTED',$this->value('SELECT workflow_status FROM arpa_division_appointment_request WHERE id=?',[$permanentRequest]),'maker edit keeps the appointment Submitted');
        $this->same($divisions[9],$this->value('SELECT arpa_division_location_id FROM arpa_division_appointment_request WHERE id=?',[$permanentRequest]),'submitted edit replaces the selected Division on the same request');
        $this->same('Submitted maker correction',$this->value('SELECT request_remarks FROM arpa_division_appointment_request WHERE id=?',[$permanentRequest]),'submitted edit saves updated request values');
        $this->same(1,$this->count('SELECT COUNT(*) FROM arpa_division_appointment_request WHERE id=?',[$permanentRequest]),'submitted edit does not create a second reservation row');
        $this->same(0,$this->count('SELECT COUNT(*) FROM arpa_division_appointment_request WHERE id=? AND arpa_division_location_id=?',[$permanentRequest,$divisions[0]]),'old submitted Division no longer reserves the timeline');
        $audit=json_decode((string)$this->value("SELECT details_json FROM audit_event WHERE target_id=? AND action_key='arpa.appointment-request.edit' ORDER BY id DESC LIMIT 1",[$permanentRequest]),true);
        $this->same($divisions[0],$audit['previous']['arpa_division_location_id']??null,'edit audit retains the previous submitted values');
        ...
        $deletedActing=$this->uuid();
        $this->pdo->prepare("INSERT INTO arpa_division_appointment_request(id,record_origin,request_type,officer_id,appointment_type,asc_location_id,arpa_division_location_id,requested_effective_from,workflow_status,created_by,deleted_at,deleted_by,delete_reason) VALUES(?,'NATIVE','APPOINTMENT',?,'ACTING',?,?,?,'SUBMITTED',?,NOW(),?,'Test deleted request')")
            ->execute([$deletedActing,$permanentOfficer,$asc,$divisions[1],$today,$actor,$actor]);
        $this->same(['ACTING','DUTY_COVERING'],$read->appointmentTypeAvailability($permanentOfficer,$today)['allowed_types'],'administratively deleted Acting request does not remove Acting from the form options');
        ...
        $this->throwsMessage(fn()=>$service->createAndSubmitDivisionAppointmentRequest($this->request($permanentOfficer,'ACTING',$asc,$divisions[1],$today),$actor),'The proposed start date overlaps an existing authoritative ARPA Division assignment period.','overlapping duplicate Acting for the same Division is blocked');
        $this->same(['ACTING','DUTY_COVERING'],$read->appointmentTypeAvailability($permanentOfficer,$today)['allowed_types'],'existing Acting for one Division leaves Acting available for another Division');
        $secondActing=$service->createAndSubmitDivisionAppointmentRequest($this->request($permanentOfficer,'ACTING',$asc,$divisions[2],$today),$actor);
        $this->same('SUBMITTED',$this->value('SELECT workflow_status FROM arpa_division_appointment_request WHERE id=?',[$secondActing]),'overlapping Acting assignments for different Divisions are allowed');
        $this->same(1,$this->count("SELECT COUNT(*) FROM arpa_division_appointment a JOIN arpa_division_appointment_request r ON r.id=a.request_id WHERE a.officer_id=? AND a.appointment_type='PERMANENT' AND a.arpa_division_location_id=? AND r.workflow_status='NATIONAL_APPROVED'",[$permanentOfficer,$divisions[9]]),'creating Acting requests leaves the existing approved Permanent appointment unchanged');
        $this->throwsMessage(fn()=>$service->createAndSubmitDivisionAppointmentRequest($this->request($permanentOfficer,'ATTEND_TO_DUTY',$asc,$divisions[3],$today),$actor),'Attend to the Duty can only be assigned to an officer who is not Permanent In Service.','Permanent-in-Service officer cannot receive Attend to the Duty');

        $firstDuty=$service->createAndSubmitDivisionAppointmentRequest($this->request($permanentOfficer,'DUTY_COVERING',$asc,$divisions[3],$today),$actor);
        $secondDuty=$service->createAndSubmitDivisionAppointmentRequest($this->request($permanentOfficer,'DUTY_COVERING',$asc,$divisions[4],$today),$actor);
        ...
        $this->throwsMessage(fn()=>$service->updateAndResubmitRequest('division',$secondDuty,$this->request($permanentOfficer,'DUTY_COVERING',$asc,$divisions[3],$today),$actor),'The proposed start date overlaps an existing authoritative ARPA Division assignment period.','submitted edit re-runs overlap and continuity validation');
        $this->same($divisions[4],$this->value('SELECT arpa_division_location_id FROM arpa_division_appointment_request WHERE id=?',[$secondDuty]),'failed submitted edit leaves the existing reservation unchanged');
        $this->throwsMessage(fn()=>$service->createAndSubmitDivisionAppointmentRequest($this->request($permanentOfficer,'DUTY_COVERING',$asc,$divisions[3],$today),$actor),'The proposed start date overlaps an existing authoritative ARPA Division assignment period.','duplicate submitted Duty Covering is blocked by the authoritative Division reservation');
        $this->createApprovedAppointment($permanentOfficer,'DUTY_COVERING',$asc,$divisions[0],$future,$actor);
        $this->throwsMessage(fn()=>$service->createAndSubmitDivisionAppointmentRequest($this->request($permanentOfficer,'DUTY_COVERING',$asc,$divisions[0],$today),$actor),'The proposed assignment period overlaps an authoritative ARPA Division assignment.','future scheduled Duty Covering cannot be bypassed by creating an unbounded earlier assignment');
