...
final class ArpaAdministrativeWorkflowTest
{
    ...
    private function exercise():void
    {
        ...
        $this->useContext($admin,'SYSTEM_ADMIN');$this->same(true,ArpaAdministrativePolicy::canCorrectDates(),'canonical dems.admin may correct historical dates');
        $identityBefore=$this->value('SELECT CONCAT_WS(\'|\',request_id,officer_id,asc_location_id,arpa_division_location_id) FROM arpa_division_appointment WHERE id=?',[$appointment]);
        $service->correctDates($appointment,['effective_from'=>'2024-12-31','effective_to'=>'2025-01-20','correction_reason'=>'Correct imported historical start'],$admin);
        $row=$service->appointmentForCorrection($appointment);$this->same('2024-12-31',$row['effective_from'],'canonical dems.admin may correct a start before the native baseline');$this->same('2024-12-31',$row['requested_effective_from'],'linked request follows a pre-baseline correction');
        $service->correctDates($appointment,['effective_from'=>'2019-08-01','effective_to'=>'2025-01-20','correction_reason'=>'Correct substantially earlier imported history'],$admin);
        $row=$service->appointmentForCorrection($appointment);$this->same('2019-08-01',$row['effective_from'],'substantially earlier historical start is accepted without a conflict');$this->same('2019-08-01',$row['requested_effective_from'],'originating request start date stays synchronized');$this->same('2025-01-20',$row['effective_to'],'canonical closure date is corrected');$this->same('2025-01-20',$row['end_request_effective_to'],'END request date stays synchronized');
        $this->same($identityBefore,$this->value('SELECT CONCAT_WS(\'|\',request_id,officer_id,asc_location_id,arpa_division_location_id) FROM arpa_division_appointment WHERE id=?',[$appointment]),'date correction does not change appointment identity or location links');
        $this->same(2,(int)$this->value("SELECT COUNT(*) FROM arpa_appointment_data_correction WHERE appointment_id=? AND correction_action='ADMIN_DATE_CORRECTION'",[$appointment]),'each direct correction is append-only audited');
        $latestAudit=(string)$this->value("SELECT before_json FROM arpa_appointment_data_correction WHERE appointment_id=? AND correction_action='ADMIN_DATE_CORRECTION' ORDER BY created_at DESC,id DESC LIMIT 1",[$appointment]);$this->same(true,str_contains($latestAudit,'2024-12-31'),'correction audit retains the previous historical start');
        ...
        $this->throws(fn()=>$service->correctDates($appointment,['effective_from'=>'2019-08-01','effective_to'=>'2025-01-20','correction_reason'=>''],$admin),'correction reason is mandatory');
        $this->throws(fn()=>$service->correctDates($appointment,['effective_from'=>'2024-01-01','effective_to'=>'2023-12-31','correction_reason'=>'Invalid ordering'],$admin),'historical correction still rejects invalid date ordering');
        $this->futureAppointment($officer,$asc,$division,$systemAdmin,'2025-02-01','2025-02-10');
        $this->throws(fn()=>$service->correctDates($appointment,['effective_from'=>'2019-08-01','effective_to'=>'2025-02-02','correction_reason'=>'Overlapping correction'],$admin),'overlapping authoritative appointment is rejected while self is excluded');
        ...
        $correctionView=(string)file_get_contents(BASE_PATH.'/app/Views/arpa_appointments/admin_date_correction.php');$this->same(false,str_contains($correctionView,'min="2025-01-01"'),'administrative correction inputs do not impose the native baseline');
        $this->throws(fn()=>\App\Services\ArpaAppointmentRules::assertNativeEffectiveDate('2024-12-31'),'normal native appointment validation retains the 2025 baseline');
    }
