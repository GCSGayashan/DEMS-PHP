...
final class ArpaAppointmentDataIssueCorrectionTest
{
    ...
    private function multipleOpenCorrection():void
    {
        ...
        $firstOfficer=$this->officer;$first=$this->legacyAppointment($this->divisions[0],'PERMANENT','2018-06-10',$metadata);$this->officer=$this->newOfficer();$secondOfficer=$this->officer;$second=$this->legacyAppointment($this->divisions[0],'ACTING','2025-02-01',$metadata+['source_row_id'=>910002]);$this->officer=$firstOfficer;
        $rowKey='DIVISION_MULTIPLE_OPEN:'.$this->divisions[0];$service=new ArpaAppointmentDataIssueCorrectionService($this->pdo);
        $issue=$service->issue($rowKey);$this->same('DIVISION_MULTIPLE_OPEN',$issue['issue_type']??null,'multiple-open fixture appears in diagnostics');$this->same(null,$issue['officer_id']??null,'multi-Officer Division issue has no false scalar owner');
        $detail=$service->detail($rowKey,$this->actor);$this->same(2,count($detail['appointments']),'grouped issue displays every involved appointment side-by-side');$this->same(2,count(array_unique(array_column($detail['appointments'],'officer_id'))),'grouped issue retains each appointment actual Officer');$this->same(2,count(array_unique(array_column($detail['appointments'],'appointment_officer_number'))),'grouped issue displays each appointment actual Officer Number');$this->same(true,$detail['correctable'],'own-ASC grouped issue is directly correctable');
        ...
        $ledger=$this->row('SELECT * FROM arpa_appointment_data_correction WHERE id=?',[$result['correction_id']]);$this->same($secondOfficer,$ledger['officer_id'],'correction ledger derives the Officer from the selected appointment');$this->same('APPOINTMENT_DATA_ISSUE',$ledger['source'],'correction ledger records the dedicated source');$this->same(true,str_contains((string)$ledger['before_json'],'legacy_history_only'),'before JSON is retained');$this->same(true,str_contains((string)$ledger['after_json'],'legacy_history_only'),'after JSON is retained');
        ...
        $profile=(new OfficerProfileService($this->pdo))->profile($secondOfficer,[],[$this->asc]);$this->same(true,in_array($result['correction_id'],array_column($profile['appointment_corrections'],'id'),true),'selected Officer Profile service exposes correction history');
