...
final class AccountChangePasswordTest
{
    ...
    public function run():int
    {
        ...
        try{
            $boundaryHash=CredentialService::hashPassword('Abcd1!xy');
            $this->same(true,password_verify('Abcd1!xy',$boundaryHash),'exactly 8-character complex password is accepted');

            $temporaryBoundaryHash=CredentialService::hashTemporaryPassword('Temp1!xy');
            $this->same(true,password_verify('Temp1!xy',$temporaryBoundaryHash),'exactly 8-character temporary password is accepted');

            $this->rejects(fn()=>CredentialService::hashPassword('Abc1!xy'),'7-character password is rejected');
            $this->rejects(fn()=>CredentialService::hashPassword('abcd1!xy'),'password without uppercase is rejected');
            $this->rejects(fn()=>CredentialService::hashPassword('ABCD1!XY'),'password without lowercase is rejected');
            $this->rejects(fn()=>CredentialService::hashPassword('Abcde!xy'),'password without number is rejected');
            $this->rejects(fn()=>CredentialService::hashPassword('Abcde1xy'),'password without symbol is rejected');
            ...
            $controller=file_get_contents(BASE_PATH.'/app/Controllers/AuthController.php');$accountRequestService=file_get_contents(BASE_PATH.'/app/Services/UserAccountRequestService.php');$view=file_get_contents(BASE_PATH.'/app/Views/auth/change_password.php');$layout=file_get_contents(BASE_PATH.'/app/Views/layouts/admin.php');$this->same(false,str_contains($controller,"['user_id']"),'self-service route accepts no target user ID');$this->same(true,str_contains($accountRequestService,'CredentialService::hashTemporaryPassword'),'new account temporary passwords use the central password policy');$this->same(true,str_contains($view,'minlength="8"'),'change-password form enforces the 8-character minimum');$this->same(true,str_contains($view,'Current Password'),'form requires current password');$this->same(true,str_contains($view,'Csrf::field()'),'form is CSRF protected');$this->same(true,str_contains($layout,'Change Password'),'account menu exposes Change Password');
