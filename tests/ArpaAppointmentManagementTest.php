...
final class ArpaAppointmentManagementTest
{
    ...
    private function testSchemaAndZeroLegacyImport(): void
    {
        ...
        foreach(['arpa_division_appointment_request','arpa_division_appointment','arpa_division_appointment_closure','arpa_subject_assignment_request','arpa_subject_assignment','arpa_subject_assignment_closure'] as $table)$this->same(0,$this->scalar("SELECT COUNT(*) FROM {$table} WHERE record_origin='NATIVE'"),"migration creates no native rows in {$table}");
    ...
    private function testTransactionalLifecycle(): void
    {
        ...
        try {
            ...
            $request=$service->createDivisionAppointmentRequest(['officer_id'=>$officer,'appointment_type'=>'PERMANENT','asc_location_id'=>$location['asc_id'],'arpa_division_location_id'=>$location['arpa_id'],'effective_from'=>$today],$creator);
            $service->workflow('division',$request,'SUBMIT','CREATOR',null,$creator);
