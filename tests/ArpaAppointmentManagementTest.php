...
final class ArpaAppointmentManagementTest
{
    ...
    private function testTransactionalLifecycle(): void
    {
        ...
        try {
            ...
            $actingDivision=$this->anotherDivision((string)$location['asc_id'],(string)$location['arpa_id']);
            $thirdDivision=$this->anotherDivision((string)$location['asc_id'],(string)$location['arpa_id'],true);
            foreach([(string)$location['arpa_id'],$actingDivision] as $division)$this->coverDivisionThroughYesterday($coverageOfficer,(string)$location['asc_id'],$division,$today,$creator);
            $this->coverDivisionThroughYesterday($coverageOfficer,(string)$location['asc_id'],$thirdDivision,$tomorrow,$creator);
            ...
            $actingAppointment=(string)$this->value("SELECT id FROM arpa_division_appointment WHERE request_id='{$actingRequest}'");
            $returnedEnd=$service->createEndRequest($actingAppointment,$today,$reason,'Returned end fixture',$creator);
            $service->workflow('division',$returnedEnd,'SUBMIT','CREATOR',null,$creator);
            $service->workflow('division',$returnedEnd,'VERIFY','ASC',null,$creator);
            $service->workflow('division',$returnedEnd,'RETURN_FOR_CORRECTION','ASC','Keep the current appointment active.',$approve);
            $this->same(0,$this->scalar("SELECT COUNT(*) FROM arpa_division_appointment_closure WHERE request_id='{$returnedEnd}'"),'returned End Appointment does not create a canonical closure');
            $rejectedEnd=$service->createEndRequest($actingAppointment,$today,$reason,'Rejected end fixture',$creator);
            $service->workflow('division',$rejectedEnd,'SUBMIT','CREATOR',null,$creator);
            $service->workflow('division',$rejectedEnd,'VERIFY','ASC',null,$creator);
            $this->same('RETURNED',$service->workflow('division',$rejectedEnd,'REJECT','ASC','Do not end this appointment.',$approve),'rejected End Appointment returns without becoming terminal');
            $this->same(0,$this->scalar("SELECT COUNT(*) FROM arpa_division_appointment_closure WHERE request_id='{$rejectedEnd}'"),'rejected End Appointment leaves the original appointment open');
            $endRequest=$service->createEndRequest($appointment,$today,$reason,'Lifecycle end',$creator);
            $service->workflow('division',$endRequest,'SUBMIT','CREATOR',null,$creator);
            $this->same(0,$this->scalar("SELECT COUNT(*) FROM arpa_division_appointment_closure WHERE request_id='{$endRequest}'"),'submitted End Appointment keeps the canonical appointment open');
            ...
            $newPermanent=$service->createDivisionAppointmentRequest(['officer_id'=>$officer,'appointment_type'=>'PERMANENT','asc_location_id'=>$location['asc_id'],'arpa_division_location_id'=>$thirdDivision,'effective_from'=>$tomorrow],$creator);
    ...
    private function coverDivisionThroughYesterday(string $coverageOfficer,string $asc,string $division,string $today,string $actor):void
    {
        $row=$this->pdo->query("SELECT a.dad_number asc_dad,a.name_en asc_name,d.dad_number arpa_dad,d.name_en arpa_name FROM location a JOIN location d ON d.id='{$division}' WHERE a.id='{$asc}'")->fetch();
        $request=(string)$this->pdo->query('SELECT UUID()')->fetchColumn();$appointment=(string)$this->pdo->query('SELECT UUID()')->fetchColumn();
        $this->pdo->prepare("INSERT INTO arpa_division_appointment_request(id,record_origin,request_type,officer_id,appointment_type,asc_location_id,arpa_division_location_id,requested_effective_from,workflow_status,created_by,finalized_by,finalized_at) VALUES(?,'NATIVE','APPOINTMENT',?,'DUTY_COVERING',?,?,'2025-01-01','NATIONAL_APPROVED',?,?,NOW())")->execute([$request,$coverageOfficer,$asc,$division,$actor,$actor]);
        $this->pdo->prepare("INSERT INTO arpa_division_appointment(id,record_origin,request_id,officer_id,appointment_type,service_permanency_snapshot,asc_location_id,arpa_division_location_id,asc_dad_snapshot,asc_name_snapshot,arpa_dad_snapshot,arpa_name_snapshot,hierarchy_snapshot_json,effective_from,approved_by,approved_at) VALUES(?,'NATIVE',?,?,'DUTY_COVERING','PERMANENT_IN_SERVICE',?,?,?,?,?,?,'{}','2025-01-01',?,NOW())")->execute([$appointment,$request,$coverageOfficer,$asc,$division,$row['asc_dad'],$row['asc_name'],$row['arpa_dad'],$row['arpa_name'],$actor]);
        $reason=(string)$this->value("SELECT id FROM arpa_appointment_end_reason WHERE active=1 ORDER BY display_order,id LIMIT 1");
        $this->pdo->prepare("INSERT INTO arpa_division_appointment_closure(id,record_origin,appointment_id,request_id,effective_to,end_reason_id,closure_kind,context_snapshot_json,approved_by,approved_at) VALUES(UUID(),'NATIVE',?,?,?,?,'DIRECT','{}',?,NOW())")->execute([$appointment,$request,date('Y-m-d',strtotime($today.' -1 day')),$reason,$actor]);
    }
