...
final class DataTableRegistry
{
    ...
    private static function officers(): array
    {
        ...
        $divisionAssignments="(SELECT GROUP_CONCAT(DISTINCT CONCAT(REPLACE(da.appointment_type,'_',' '),' - ',da.arpa_name_snapshot) ORDER BY da.appointment_type,da.arpa_name_snapshot SEPARATOR '; ') FROM arpa_division_appointment da JOIN arpa_division_appointment_request dr ON dr.id=da.request_id LEFT JOIN arpa_division_appointment_closure dc ON dc.appointment_id=da.id ".($access['with']!==''?'JOIN scoped_locations dva ON dva.id=da.asc_location_id ':'')."WHERE da.officer_id=o.id AND da.legacy_history_only=0 AND (da.record_origin='NATIVE' OR (da.record_origin='LEGACY_IMPORT' AND dr.workflow_status IN('DISTRICT_APPROVED','NATIONAL_VERIFIED','NATIONAL_APPROVED'))) AND da.effective_from<=CURRENT_DATE() AND (dc.effective_to IS NULL OR dc.effective_to>=CURRENT_DATE()))";
    ...
    private static function arpaDivisionAppointments(): array
    {
        ...
        return [
            ...
            'filters' => [
                ...
                'status' => ['allowed'=>['ACTIVE','SCHEDULED','ENDED'],'build'=>fn($v)=>["{$status}=?",[$v]],'ui'=>['label'=>'Operational Status','options'=>['ACTIVE'=>'Current','SCHEDULED'=>'Scheduled','ENDED'=>'Ended']]],
            ...
            'columns' => [
                ...
                self::col('Operational Status','operational_status',$status,fn($r)=>DataTableFormat::badge($r['operational_status']==='ACTIVE'?'CURRENT':$r['operational_status']),fn($r)=>$r['operational_status']==='ACTIVE'?'Current':DataTableFormat::enumLabel($r['operational_status'])),
                self::col('Approval Status','workflow_status','req.workflow_status',fn($r)=>DataTableFormat::badge($r['workflow_status'])),
                self::col('Source','record_origin','a.record_origin',fn($r)=>DataTableFormat::badge($r['record_origin']==='NATIVE'?'Native Appointment':'Imported Record')),
    ...
    private static function arpaRequestList(?array $queueAccess=null):array
    {
        ...
        return [
            ...
            'count'=>'r.id','baseWhere'=>['r.deleted_at IS NULL','r.legacy_history_only=0',"r.record_origin='NATIVE'"],'baseParams'=>$geo['params'],'searchable'=>['o.dad_number','o.name_with_initials','o.nic','asc_l.name_en','arpa.name_en','r.appointment_type','r.workflow_status'],
            'filters'=>[
