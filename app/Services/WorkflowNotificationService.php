...
final class WorkflowNotificationService
{
    ...
    public function actionForPermission(string $permission,?string $locationId,string $module,string $title,string $message,string $entityType,string $entityId,string $stage,string $url,string $actorId,array $roleCodes=[],bool $excludeActor=true):int
    {
        $excludeUserId=$excludeActor?$actorId:null;
        $n=0;foreach($this->recipients->forPermission($permission,$locationId,$excludeUserId,$roleCodes) as $r){$this->notifications->createActionRequired((string)$r['user_id'],$module,$title,$message,$entityType,$entityId,$stage,$url,'Review','HIGH',$actorId,(string)$r['role_assignment_id'],$r['scope_assignment_id']?:(null));$n++;}return $n;
    }
