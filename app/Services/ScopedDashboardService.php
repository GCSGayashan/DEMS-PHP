...
final class ScopedDashboardService
{
    ...
    public function dashboard(string $userId,bool $includeLegacy=false):array
    {
        ...
        $metricsSql=$with." SELECT
          ...
          ((SELECT COUNT(*) FROM arpa_division_appointment_request r JOIN visible_locations vl ON vl.id=r.asc_location_id WHERE r.deleted_at IS NULL AND r.workflow_status NOT IN('NATIONAL_APPROVED','REJECTED'))+(SELECT COUNT(*) FROM arpa_subject_assignment_request r JOIN visible_locations vl ON vl.id=r.asc_location_id WHERE r.workflow_status NOT IN('NATIONAL_APPROVED','REJECTED'))) pending_requests,
