...
final class ArpaAppointmentReadService
{
    ...
    private static function vacancyRequestExistsSql(
        ...
        return "EXISTS(SELECT 1 FROM arpa_division_appointment_request vacancy_r
                  WHERE vacancy_r.deleted_at IS NULL AND vacancy_r.arpa_division_location_id={$divisionExpression}
    ...
    public static function issueSource(string $divisionAppointmentTable = 'arpa_division_appointment'): string
    {
        ...
        $unions[]="SELECT CONCAT('LEGACY_HISTORICAL_EXCEPTION:',a.id),'LEGACY_HISTORICAL_EXCEPTION','HISTORICAL_EXCEPTION',a.officer_id,o.dad_number,o.name_with_initials,o.nic,a.asc_location_id,a.asc_name_snapshot,a.arpa_name_snapshot,a.appointment_type,CONCAT(a.effective_from,' to Open'),a.id,a.record_origin,CONCAT('Open imported appointment is preserved as a non-operational exception: ',COALESCE(JSON_UNQUOTE(a.legacy_exception_codes_json),'[]')),'Confirm whether this is genuine history or the authoritative current assignment.' FROM arpa_division_appointment a JOIN officer o ON o.id=a.officer_id LEFT JOIN arpa_division_appointment_closure c ON c.appointment_id=a.id WHERE c.id IS NULL AND a.record_origin='LEGACY_IMPORT' AND a.legacy_history_only=1 AND a.legacy_exception=1 AND NOT EXISTS(SELECT 1 FROM arpa_appointment_data_correction dc WHERE dc.appointment_id=a.id AND dc.correction_action IN('RESOLVE_CANONICAL_ASSIGNMENT','KEEP_AS_HISTORICAL_EXCEPTION') AND dc.resolution_status IN('RESOLVED_BY_CORRECTION','KEPT_HISTORICAL_EXCEPTION'))";
        $unions[]="SELECT CONCAT('LEGACY_HISTORICAL_EXCEPTION:',r.id),'LEGACY_HISTORICAL_EXCEPTION','HISTORICAL_EXCEPTION',r.officer_id,o.dad_number,o.name_with_initials,o.nic,r.asc_location_id,asc_l.name_en,arpa.name_en,r.appointment_type,CONCAT(COALESCE(r.requested_effective_from,'Unknown'),' to ',COALESCE(r.requested_effective_to,'Open')),r.id,r.record_origin,CONCAT('Legacy record is preserved as a non-operational exception: ',COALESCE(JSON_UNQUOTE(r.legacy_exception_codes_json),'[]')),'Review immutable source provenance; do not activate or rewrite the historical record.' FROM arpa_division_appointment_request r JOIN officer o ON o.id=r.officer_id LEFT JOIN location asc_l ON asc_l.id=r.asc_location_id LEFT JOIN location arpa ON arpa.id=r.arpa_division_location_id WHERE r.deleted_at IS NULL AND r.record_origin='LEGACY_IMPORT' AND r.legacy_exception=1 AND NOT EXISTS(SELECT 1 FROM arpa_division_appointment a WHERE a.request_id=r.id)";
