...
final class NotificationBackfillService
{
    ...
    public function run(bool $execute=false):array
    {
        ...
        foreach(['division'=>'arpa_division_appointment_request','subject'=>'arpa_subject_assignment_request'] as $entity=>$table){$rows=$this->pdo->query("SELECT id,asc_location_id,workflow_status,created_by,updated_by,request_type FROM {$table} WHERE record_origin='NATIVE' AND workflow_status IN('SUBMITTED','ASC_VERIFIED','ASC_APPROVED','DISTRICT_VERIFIED','DISTRICT_APPROVED','NATIONAL_VERIFIED','RETURNED')")->fetchAll();$counts['arpa']+=count($rows);if($execute)foreach($rows as $r){$map=match($r['workflow_status']){'SUBMITTED'=>['arpa.appointment.asc-verify',['ASC_SUBJECT_OFFICER']],'ASC_VERIFIED'=>['arpa.appointment.asc-approve',['ASC_ADMIN']],'ASC_APPROVED'=>['arpa.appointment.district-verify',['DISTRICT_SUBJECT_OFFICER']],'DISTRICT_VERIFIED'=>['arpa.appointment.district-approve',['DISTRICT_ADMIN']],'DISTRICT_APPROVED'=>['arpa.appointment.national-verify',['NATIONAL_SUBJECT_OFFICER']],'NATIONAL_VERIFIED'=>['arpa.appointment.national-approve',['NATIONAL_ADMIN']],default=>null};if($map)$counts['notifications_created']+=$w->actionForPermission($map[0],$r['asc_location_id'],'ARPA_APPOINTMENT','ARPA Workflow Action Required','An ARPA workflow item requires action.',strtoupper('ARPA_'.$entity.'_REQUEST'),$r['id'],$r['workflow_status'],'/hr/arpa-appointments/requests/'.$entity.'/'.$r['id'],$r['updated_by']?:$r['created_by'],$map[1]);elseif($r['workflow_status']==='RETURNED'&&!empty($r['created_by'])){$w->actionForUser($r['created_by'],'ARPA_APPOINTMENT','ARPA Request Returned for Correction','Correction is required.',strtoupper('ARPA_'.$entity.'_REQUEST'),$r['id'],'CORRECTION','/hr/arpa-appointments/requests/'.$entity.'/'.$r['id'].'/edit',$r['updated_by']?:$r['created_by']);$counts['notifications_created']++;}}}
        $generic=[
