...
final class LegacyArpaAppointmentPreviewController extends Controller
{
    ...
    private function scopedOfficerOptions(string $userId):array{$restricted=ScopeService::requiresGeographicRestriction($userId);$with=$restricted?ScopeService::visibleLocationsCte():'';$join=$restricted?' JOIN visible_locations vl ON vl.id=p.asc_location_id':'';$s=Database::pdo()->prepare($with." SELECT DISTINCT o.id,o.dad_number,o.name_with_initials FROM legacy_arpa_appointment_preview p{$join} JOIN officer o ON o.id=p.officer_id WHERE p.active=1 ORDER BY o.name_with_initials");$s->execute($restricted?[$userId]:[]);return $s->fetchAll();}
