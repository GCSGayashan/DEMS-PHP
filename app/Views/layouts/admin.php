...
<?php if($activeContext): ?><div class="dropdown"><button class="btn btn-link text-white text-decoration-none dropdown-toggle p-0 text-start" type="button" data-bs-toggle="dropdown" aria-expanded="false"><span class="d-block small opacity-75">Current Context</span><span class="fw-semibold"><?= e($activeContext['role_code']) ?> <span class="d-none d-md-inline">| <?= e($activeContext['location_label']) ?></span></span></button><ul class="dropdown-menu dropdown-menu-end" style="min-width:20rem"><li><h6 class="dropdown-header">Switch Working Context</h6></li><?php foreach($availableContexts as $context): $selected=$context['role_assignment_id']===$activeContext['role_assignment_id']&&($context['scope_assignment_id']??null)===($activeContext['scope_assignment_id']??null); ?><li><form method="post" action="<?= e(url('select-context')) ?>" class="m-0"><?= Csrf::field() ?><input type="hidden" name="role_assignment_id" value="<?= e($context['role_assignment_id']) ?>"><input type="hidden" name="scope_assignment_id" value="<?= e($context['scope_assignment_id']??'') ?>"><button class="dropdown-item<?= $selected?' active':'' ?>" type="submit"><span class="d-block fw-semibold"><?= e($context['role_name']) ?></span><span class="small<?= $selected?'':' text-muted' ?>"><?= e($context['location_label']) ?></span></button></form></li><?php endforeach; ?></ul></div><?php endif; ?>
<span class="d-none d-lg-inline"><i class="bi bi-globe2"></i> English</span><div class="dropdown"><button class="btn btn-link text-white text-decoration-none dropdown-toggle p-0" type="button" data-bs-toggle="dropdown" aria-expanded="false"><i class="bi bi-person-fill"></i> <?= e($user['username']??'User') ?></button><ul class="dropdown-menu dropdown-menu-end"><li><a class="dropdown-item" href="<?= e(url('account/change-password')) ?>"><i class="bi bi-key me-2"></i>Change Password</a></li><li><a class="dropdown-item" href="<?= e(url('select-context')) ?>"><i class="bi bi-person-badge me-2"></i>Working Contexts</a></li><li><hr class="dropdown-divider"></li><li><form method="post" action="<?= e(url('logout')) ?>" class="m-0"><?= Csrf::field() ?><button class="dropdown-item" type="submit"><i class="bi bi-box-arrow-right me-2"></i>Logout</button></form></li></ul></div></div></header>
