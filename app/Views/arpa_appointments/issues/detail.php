...
<?php elseif($correctable&&$issue['issue_type']==='LEGACY_HISTORICAL_EXCEPTION'&&$appointments): ?><div class="card border-warning mb-4"><div class="card-body"><div class="alert alert-warning"><strong>Confirm the canonical business state.</strong><br>Making this appointment authoritative will preserve its legacy provenance and correction audit. Any exact matching native reservation will be retained as administratively superseded, not physically deleted.</div><h2 class="h5">Resolve Imported Appointment</h2><form method="post" action="<?= e(url('hr/arpa-appointments/issues/'.rawurlencode($issue['row_key']).'/correct')) ?>"><?= Csrf::field() ?><input type="hidden" name="appointment_id" value="<?= e($appointments[0]['id']) ?>"><div class="row g-3"><div class="col-12"><label class="form-label">Supporting Record</label><input class="form-control" name="evidence_reference" maxlength="500"></div><div class="col-12"><label class="form-label">Reason for Correction *</label><input class="form-control" name="correction_reason" maxlength="500" required></div><div class="col-12"><label class="form-label">Correction Notes</label><textarea class="form-control" name="remarks" rows="3"></textarea></div><div class="col-12 d-flex gap-2 justify-content-end"><button class="btn btn-outline-secondary" type="submit" name="correction_action" value="KEEP_AS_HISTORICAL_EXCEPTION">Keep as Historical Exception</button><button class="btn btn-warning" type="submit" name="correction_action" value="RESOLVE_CANONICAL_ASSIGNMENT">Make Authoritative / Current</button></div></div></form></div></div>
