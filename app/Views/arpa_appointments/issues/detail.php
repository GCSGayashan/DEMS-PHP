...
<?php foreach($appointments as $a): ?><tr><td><?= e($issue['officer_number'].' - '.$issue['officer_name']) ?></td><td><?= e($a['asc_name_snapshot']) ?><br><strong><?= e($a['arpa_dad_snapshot'].' - '.$a['arpa_name_snapshot']) ?></strong></td><td><?= e(DataTableFormat::enumLabel($a['appointment_type'])) ?></td><td><?= DataTableFormat::date($a['effective_from']) ?> to <?= DataTableFormat::date($a['effective_to'],'Open') ?></td><td><?= (int)$a['legacy_history_only']===1?DataTableFormat::badge('Old Record - History Only'):DataTableFormat::badge('Current Appointment') ?> <?= (int)$a['legacy_exception']===1?DataTableFormat::badge('Old Data Warning'):'' ?></td><td><a class="btn btn-sm btn-outline-primary" href="<?= e(url('hr/arpa-appointments/divisions/'.$a['id'])) ?>">View Appointment</a></td></tr><?php endforeach; ?>
